...
namespace ThinSdk.Neo.SmartContract.Manifest
{
    ...
    public class ContractManifest : ISerializable
    {
        ...
        public static ContractManifest LoadContractManifest(string _manifestFilePath)
        {
            var info = new FileInfo(_manifestFilePath);
            if (!info.Exists || info.Length >= Transaction.MaxTransactionSize)
            {
                throw new System.Exception(nameof(_manifestFilePath));
            }
            var manifest = ContractManifest.Parse(File.ReadAllText(_manifestFilePath));
            return manifest;
        }
    }
}
