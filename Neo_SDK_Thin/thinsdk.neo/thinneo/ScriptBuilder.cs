    ...
    public class ScriptBuilder : IDisposable
    {
        ...
        public static byte[] GetParamBytes(string str)
        {
            ...
            else if (str.IndexOf("(address)") == 0)
            {
                ...
                var hex = ThinNeo.Helper_NEO.GetScriptHash_FromAddress(addr);
                return hex;
            }
        ...
        public ScriptBuilder EmitParamJson(JToken param)
        {
            if ( param.Type == JTokenType.Boolean || param.Type == JTokenType.Float || param.Type == JTokenType.Float || param.Type == JTokenType.Null)//bool 或小整数
            {
                var num = param;
                if (num.Type == JTokenType.Boolean)
                {
                    this.EmitPushBool((bool)num);
                }
                else
                {
                    this.EmitPushNumber((int)num);
                }
            }
            else if (param.Type== JTokenType.Array)
            {
                var list = (param as JArray);
                for (var i = list.Count - 1; i >= 0; i--)
                {
                    EmitParamJson(list[i]);
                }
                this.EmitPushNumber(list.Count);
                this.Emit(ThinNeo.VM.OpCode.PACK);
            }
            else if (param.Type== JTokenType.String)//复杂格式
            {
