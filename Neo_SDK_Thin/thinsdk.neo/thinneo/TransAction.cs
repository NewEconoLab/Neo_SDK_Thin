    ...
    public class Transaction
    {
        ...
        public void AddWitness(byte[] signdata, byte[] pubkey, string addrs)
        {
            {//额外的验证
                ...
                bool bsign = ThinNeo.Helper_NEO.VerifySignature(msg, signdata, pubkey);
                if (bsign == false)
                    throw new Exception("wrong sign");

                var addr = ThinNeo.Helper_NEO.GetAddress_FromPublicKey(pubkey);
                if (addr != addrs)
                    throw new Exception("wrong script");
            }

            var vscript = ThinNeo.Helper_NEO.GetAddressScript_FromPublicKey(pubkey);
